        ...
        for stock_name_one in stock_name:
                if stock_name_one in question_one['question']:
                    if stock_name_one in stock_mapping:
                        ...
                        file_form_data = file_form[stock_mapping[stock_name_one][0]]
                        for file_form_one in file_form_data:
                            file_form_line = file_form_one.split("\n")
                            for file_form_unit in file_form_line:
                                file_form_cell = file_form_unit.split("|")
                                for file_form_cell_one in file_form_cell:
                                    if len(file_form_cell_one) > 1:
                                        if file_form_cell_one in question_one['question']:
                                            all_texts = [
                                                "[Round 0]\n 根据表格:" + file_form_one + " 寻找问题：" + question_one[
                                                    'question'] + "    \n答：",
                                            ]
        if len(all_texts) == 0:
            all_texts = [
